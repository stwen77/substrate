...
/// changing the `Inherent` constructor ☹
#[cfg_attr(feature = "std", derive(Serialize, Debug))]
#[derive(Encode, Decode, PartialEq, Eq, Clone)]
pub enum RawLog<T> {
	/// AuRa inherent digests
	Inherent([u8; 4], Vec<u8>),
	/// An absurd variant, used only at the type level.
	Absurd(Absurd, PhantomData<T>)
}

struct Inherent(pub [u8; 4], pub Vec<u8>);

impl<T> From<RawLog<T>> for Inherent {
	fn from(t: RawLog<T>) -> Inherent {
		match t {
			RawLog::Inherent(a, b) => Inherent(a, b),
